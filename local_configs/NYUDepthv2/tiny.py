...
# Dataset config
"""Dataset Path"""
C.dataset_name = 'flower_data'
C.dataset_path = osp.join(C.root_dir, 'flower_data')
...
C.x_root_folder = osp.join(C.dataset_path, 'vision')
C.x_format = '.jpg'
C.x_is_single_channel = True  # True for raw depth, thermal and aolp/dolp(not aolp/dolp tri) input
C.train_source = osp.join(C.dataset_path, "train.txt")
C.eval_source = osp.join(C.dataset_path, "test.txt")
C.is_test = True
C.num_train_imgs = 2936
C.num_eval_imgs = 734
C.num_classes = 40
C.class_names = ['daisy', 'dandelion', 'roses', 'sunflowers', 'tulips']
...
C.checkpoint_dir = osp.abspath(osp.join(C.log_dir,
                                        "checkpoint"))  # '/mnt/sda/repos/2023_RGBX/pretrained/'#osp.abspath(osp.join(C.log_dir, "checkpoint"))
...
add_path(os.path.join(C.root_dir))
