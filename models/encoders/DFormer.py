...
class attention(nn.Module):
    ...
    def forward(self, x, x_e):
        ...
        if self.window != 0:
            ...
            attn = (attn @ v).reshape(B, self.num_head, self.window, self.window, C // self.num_head // 2).permute(0, 1,
                                                                                                                   4, 2,
                                                                                                                   3).reshape(
...
class StemModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1x1 = nn.Conv2d(in_channels, out_channels // 4, kernel_size=1, stride=2, padding=0)
        self.conv3x3 = nn.Conv2d(in_channels, out_channels // 4, kernel_size=3, stride=2, padding=1)
        self.conv5x5 = nn.Conv2d(in_channels, out_channels // 4, kernel_size=5, stride=2, padding=2)
        self.conv7x7 = nn.Conv2d(in_channels, out_channels // 4, kernel_size=7, stride=2, padding=3)
