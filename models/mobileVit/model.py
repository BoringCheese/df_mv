...
class MobileViTBlock(nn.Module):
    ...
    def unfolding(self, x: Tensor) -> Tuple[Tensor, Dict]:
        ...
        x = x.transpose(1, 2)
        # [B * C * n_h, n_w, p_h, p_w] -> [B, C, N, P] where P = p_h * p_w and N = n_h * n_w00
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        self.fct = nn.Sequential(
            nn.Linear(4 * 1000, 12800),  # 输入维度为4*5=20，隐藏层节点数为64
            nn.Linear(12800, 6400),
            nn.Linear(6400, 1000),  # 隐藏层节点数为64，下一个隐藏层节点数为32
